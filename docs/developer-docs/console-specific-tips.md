...
When excavating the memory of these old consoles, it helps to better understand the original hardware. If you familiarize yourself with the quirks of your favorite console, set making will be easier and more accurate.

[[toc]]

## Amstrad CPC

Games written in BASIC will often use Microsoft Binary Format for numeric values. This is a floating point data. You can filter against the `MBF32 LE` data size on this system to find these values.

## Apple II

Games written in BASIC will often use Microsoft Binary Format for numeric values. This is a floating point data. You can filter against the `MBF32` data size on this system to find these values.
...
- Identification of the game/disc:
   - Serial is located at `0x00000000` as an ASCII string.
...
The RAM from `0x0000-0xFFFF` is reserved for the kernel. The boot executable name appears to reliably be placed at `0x9E18` AND `0xB8B0`. The boot executable name almost always contains the disc serial which is unique per region, so this can often be used to determine which version of a game the player is using.
...
- Identification of a retail game/disc:
   ...
   - Revision Number is stored at `0x00000007`.
   - You can use these to determine the specific disc loaded.
- WiiWare game ID is located at `0x00003180` as a 4-byte ASCII string.
