...
- **请勿使用漏洞！**：不要制作需要漏洞的成就。漏洞在大多数情况下会破坏和 destabilize 游戏内存和 RAM。它们会破坏整个成就集。然而，明智的做法是编写阻止玩家使用漏洞轻松获得 mastery 的墙壁代码。
...
1. `Value 1 = Value 0`；这是为了防止成就 ever 为真，因为值 0 永远不等于 1。
