...
- **环码**：某些系统，如世嘉土星，有不同“环码”的游戏多个版本。这些是 stamped 在实体光盘内环上的字母数字标识符。在大多数情况下，它们对游戏或甚至哈希没有功能影响，但如版本所述，请先检查是否有已知差异。
...
- **无 Mod**：使用干净的 ROM。不要使用带有 modded/自定义启动画面的 ROM，如 Mode7 和 RisingSun 所具有的，因为它会改变 RAM 值并使用户使用自己合法转储的 ROM 的能力复杂化。
...
[TOSEC](https://www.tosecdev.org/) 是另一个限制较少的 preservation 组织。它们的哈希通常与 Redump 的光盘匹配，但 frequently 包含验证较不彻底的转储。对于 No-Intro 和/或 Redump 验证不可用的软盘、磁带和较不知名系统，TOSEC 是良好的备选选择。
...
有时翻译补丁可能产生与链接哈希 critically 不同的内存。所有链接哈希预期由*整个*成就集完全支持。若您无法个人保证这一点，请不要链接哈希。
...
- 错误修复；
- 轻度或大量翻译；
- 轻度或大量 cosmetic 更改，包括“冒犯性”元素被审查、音效更改、授权素材被通用素材替换和/或未知角色被任天堂化（例如 Tetris Attack、Super Mario Bros. 2）；
- 关卡添加、修改或移除；
- 游戏物理和游戏机制修改；
- 游戏模式、难度级别和游戏选项添加或移除；
- 作弊码或调试模式添加或移除；
